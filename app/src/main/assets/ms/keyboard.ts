...
declare const Guppy;
declare const rendera11ystring;
...
const myPostMessage = (event: string, data: any) => {
...
window.editorInsertString = (value: string) => {
    guppy.activate();
...
window.editorInsertSymbol = (value: string) => {
    guppy.activate();
    guppy.engine.insert_symbol(value);
    guppy.render(true);
};

window.editorSetContent = (value: string) => {
    guppy.activate();
...
window.enterPower = (value: string) => {
...
export default editorInstanceName;